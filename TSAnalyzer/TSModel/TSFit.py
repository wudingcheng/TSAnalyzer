...
class TSFit(object):
    ...
    def ts_fit(self, **kwargs):
        ...
        timespan = self.t.max() - self.t.min()
    ...
    def _ts_fit(self, **kwargs):
        ...
        if offsets is not None and len(offsets) > 0:
            # B = np.zeros((len(offsets), nepochs))
            # for i, t_i in enumerate(offsets):
            #     B[i, t >= t_i] = 1
            # A = np.vstack((A, B))
        ...
        result['p'] = p
        result['v'] = v
        number = len(kwargs.get('offsets', [])) + 2 * \
            len(kwargs.get('psdecays', []))
        if number != 0:
            temp_p = np.copy(p)
            temp_p[:-number] = 0
        ...
        if kwargs.get('full', False):
            ...
            wrms = np.sqrt(
                (nepochs / (nepochs - A.shape[1])) * chi_square / sum(w))
            result['nrms'] = nrms
            result['wrms'] = wrms
        return result

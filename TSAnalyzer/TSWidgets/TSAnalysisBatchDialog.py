...
class TSAnalysisBatchDialog(QDialog, AnalysisBatchDialog):
    def __init__(self, parent=None):
        ...
        for i in range(len(buttons)):
            ...
            # buttons[i].clicked.connect(lambda i=i: self.click_dir_button(edits[i]))
            buttons[i].setIcon(QIcon(":/TSResource/images/directory"))
        self.outlierDirButton.clicked.connect(
            lambda: self.click_dir_button(self.outlierDirEdit))
        self.cmeButton.clicked.connect(
            lambda: self.click_dir_button(self.cmeEdit))
        self.logDirButton.clicked.connect(
            lambda: self.click_dir_button(self.logDirEdit))
        self.saveDirButton.clicked.connect(
            lambda: self.click_dir_button(self.saveDirEdit))
    ...
    def on_selectButton_clicked(self):
        # files = self.dialog.getOpenFileNames(
        #     self, "Choose Time Series Files", '',
    ...
    def on_listWidget_itemDoubleClicked(self):
        ...
        reply = QMessageBox.question(
            self,
            'Delete',
            "Delete %s from list?" % self.listWidget.item(
                ind).text(),

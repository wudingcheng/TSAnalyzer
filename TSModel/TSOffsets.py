...
def write_offsets(offset, file='offsets.json'):
    try:
        ...
        offsets = {}
    for key, item in offset.iteritems():
        try:
...
def offset_to_fit(offset, component="north"):
    offsets = []
    psdecays = []
    for date, components in offset.iteritems():
        ...
        if com == 'eq' or com == 'ep':
            ...
            psdecays.append([date, date + timedelta(days=int(com.split()[-1])), 0])
        elif 'log' in com:
            psdecays.append([date, date + timedelta(days=int(com.split()[-1])), 1])

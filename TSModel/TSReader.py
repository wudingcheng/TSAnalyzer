...
class TSReader(QThread):
        ...
        with open(self.filename, 'r') as f:
            ...
            for line in lines:
                ...
                if line.startswith("# columns_index"):
                    col_indexs = map(int, line.strip().split()[2:])
    ...
    def _read_mjd(self, index='mjd'):
        ...
        self.df = pd.read_csv(self.filename, header=None, sep="\s+", comment='#',
                              names=names, usecols=cols, skiprows=self.skiprows)
        timeindex = map(mjd2date, self.df['mjd'])
        del self.df['mjd']
    ...
    def _read_file(self):
        ...
        self.df = pd.read_csv(self.filename, header=None, sep="\s+", comment='#',
                              names=names, usecols=cols,
                              index_col="datetime", parse_dates=self._parser_dates,
                              date_parser=self._date_parser, skiprows=self.skiprows)
    ...
    def read_file(self, filename):
        ...
        if self.scale > 1:
            self.df[self.df.columns[1:]] = self.df[self.df.columns[1:]].apply(lambda i: i * self.scale)
    ...
    def interpolate(self, df, method='mean', order=3):
        if method != 'mean':
            df[self.columns] = df[self.columns].interpolate(method=method, order=order)
    ...
    def outliers(self, **kwargs):
        ...
        if iqr_factor:
            ...
            for col in self.columns:
                ...
                low = median - qrange
                high = median + qrange
                ind = np.logical_or(residual.values < low.values, residual.values > high.values)
                ind2 = (residual - residual.mean()).abs() > iqr_factor * residual.std()
    ...
    def fit(self, **kwargs):
        ...
        for col in self.columns:
            ...
            if offsets_dict is not None:
                offset_dates, psdecay_dates = offset_to_fit(offsets_dict, component=col)
                ...
                for i in psdecay_dates:
                    start, end = self.df.index.searchsorted(i[0]), self.df.index.searchsorted(i[1])
...
# index_formats: %s\n''' % (self.time_unit, self.unit,
                            ...
                            ' '.join(map(str, range(0, len(df.columns) + 2))),
